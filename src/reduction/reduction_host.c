...
void create_kernels()
{
  ...
  utr_ocl_create_kernel_dev_type( platform_index, UTC_OCL_DEVICE_GPU, kernel_index,
				  // kernel name:         , file:
				  "reduction_single_kernel", "reduction_single.cl", monitor);
  
  // create the first kernel for GPU
  kernel_index = 1; 
  utr_ocl_create_kernel_dev_type( platform_index, UTC_OCL_DEVICE_GPU, kernel_index,
				  // kernel name:         , file:
				  "reduction_conflict_kernel", "reduction_conflict.cl", monitor);
  
  // create the first kernel for GPU
  kernel_index = 2; 
  utr_ocl_create_kernel_dev_type( platform_index, UTC_OCL_DEVICE_GPU, kernel_index,
				  // kernel name:         , file:
				  "reduction_opt_kernel", "reduction_opt.cl", monitor);
...
#define VECTOR_SIZE MULT_MAX*WORK_GROUP_SIZE
  ...
  SCALAR* A = (SCALAR *) malloc(VECTOR_SIZE*sizeof(SCALAR));
  SCALAR* B = (SCALAR *) malloc(VECTOR_SIZE*sizeof(SCALAR));
  SCALAR  sc_prod;
  SCALAR  sc_prod_test;
  
  for(i=0;i<VECTOR_SIZE;i++) A[i]=(1.0*i)/(VECTOR_SIZE);
  for(i=0;i<VECTOR_SIZE;i++) B[i]=(1.0*i)/(VECTOR_SIZE);

  // working size <= VECTOR_SIZE
  int size_abc = VECTOR_SIZE;
  //int size_abc = WORK_GROUP_SIZE;

  printf("\n------------Starting OpenMP execution phase----------------\n");
  printf("\nArrays size: %d (%d MB)\n", VECTOR_SIZE, VECTOR_SIZE*sizeof(SCALAR)/1024/1024);
  t_begin = time_clock();

#pragma omp parallel for default(none) shared(size_abc, A, B) reduction(+:sc_prod_test)
  for(i=0;i<size_abc;i++) {
    sc_prod_test += A[i]*B[i];
  }

#ifdef time_measurments
  t_end = time_clock();
  printf("EXECUTION TIME: executing standard OpenMP loop: %lf (sc_prod = %lf)\n", 
	 t_end-t_begin, sc_prod_test);
  printf("\tNumber of operations %d, performance %lf GFlops\n",
	 2*VECTOR_SIZE, 2*VECTOR_SIZE / (t_end-t_begin) * 1e-9);
  printf("\tGBytes transferred to/from processor %lf, speed %lf GB/s\n",
	 2*VECTOR_SIZE*sizeof(SCALAR)*1e-9, 
	 2*VECTOR_SIZE*sizeof(SCALAR)/(t_end-t_begin)*1e-9);
#endif
  ...
  for(idev=0; idev<platform_struct.number_of_devices; idev++){
    ...
    // verify result 
    verify_result( sc_prod_test, sc_prod);
    ...
    if(context == NULL || command_queue == NULL || kernel == NULL){ 
      ...
      // call routine to perform the actual work....
      // pass OpenCL parameters and host input/output data
            reduction_host_2(kernel_index, A, B, &sc_prod, size_abc, 
      		   nr_threads, WORK_GROUP_SIZE, context, kernel, command_queue);
...
// we create an array of size nr_work_groups to store work_group results
  ...
  clEnqueueReadBuffer(queue, d_C, CL_TRUE, 0, size_bytes_C, C, 0, 0, &ndrEvt); 
  ...
  printf("\nTime for sending C from GPU memory:  %lf (internal %lf)\n", 
	 t2-t1, time_internal);
  printf("\nBandwidth %lf GB/s (size %ul)\n", 
	 size_bytes_C/(time_internal)/1024/1024/1024, size_bytes_C);
		  ...
		  )
{
  ...
  int result_OK = 1;
  int i,j;
  if(fabs((result - result_compare)/result_compare)>1.e-6) {
    printf("wrong result - %16.8f != %16.8f\n", result, result_compare);
    result_OK = 0;
  }
  printf("\n\t\tVerifying results: ");
  if(result_OK) {
